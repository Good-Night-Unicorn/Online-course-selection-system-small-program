<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="search-form round">
				<input v-model="searchForm.xueshengName" type="text" placeholder="学生姓名"></input>
		</view>
...
<view class="list" :style='{"borderRadius":0,"backgroundColor":"rgba(255, 255, 255, 0)"}'>
    <view style="margin: 10rpx 10rpx 10rpx 10rpx;"
        ...
        <image :style='{"borderRadius":"40rpx","width":"50%","height":"200rpx"}' mode="aspectFill" class="listmpic"
               :src="product.xueshengPhoto?product.xueshengPhoto.split(',')[0]:''"></image>
        <view class="listmr">
            <view class="col3 f30 elip mb15"
                  :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"rgba(255, 255, 255, 1)","textAlign":"center"}'>
                {{product.xueshengName}}
            </view>
        </view>
           <view style="display: flex;justify-content: space-between;float: left;margin-top: 20%;">
            <text style="margin-left: 50rpx;" v-if="isAuth('xuesheng','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">
                修改
            </text>
            <text v-if="isAuth('xuesheng','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('xuesheng','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		methods: {
			...
			async search() {
				...
				if (this.searchForm.xueshengName) {
                    searchForm['xueshengName'] = this.searchForm.xueshengName
                }
				if (this.searchForm.xueshengName) {
					searchForm['xueshengName'] = this.searchForm.xueshengName
				}
